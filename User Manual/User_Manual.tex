...
\item Double left-click on Microdroplet\_Impact\_Video\_Processor.mlappinstall. \\
...
\item From the dropdown menu, left-click "Microdroplet Impact Video Processor" from the "MY APPS" section.
\begin{figure}[H]
\centering
...
A for loop is again used to find the radius in each frame. The first step in the loop is to find the extrema points of the droplet. Then the "spreadLeft" and "spreadRight" datasets are changed to reflect the new radius. The radius is calculated by finding the difference in pixels between the extreme points and the center. This assumes that the droplet has fallen straight down in the video. The max radii are determined using the max() function on the radii datasets.
