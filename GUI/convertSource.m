...
converted_Source_Collection = zeros(a,b,3,d);
...
elseif islogical(image_Collection) == 1
    for n = 1:3
        converted_Source_Collection(:,:,n,:) = uint8(image_Collection(:,:,1,:));
    end
elseif c == 1
    for n = 1:3
        converted_Source_Collection(:,:,n,:) = uint8(image_Collection(:,:,1,:));
...
elseif (c == 3)
    if isa(sourceM,'uint8') == 0
        converted_Source_Collection(:,:,:,:) = uint8(image_Collection(:,:,:,:));
    end
else
    error('Unexpected Source Matrix format. Please provide a grayscale image, an RGB image, or a binary image. ')
end
